...
func main() {
	...
	rootCertFile, err := os.ReadFile(path.Join(*outputPath, "test_root_ca_cert.pem"))
	if err != nil {
		klog.Fatalf("Failed to read root CA certificate: %v", err)
	}
	var block *pem.Block
	block, rootCertFile = pem.Decode(rootCertFile)

	rootCert, err := x509.ParseCertificate(block.Bytes)
	if err != nil {
		klog.Fatalf("error parsing PEM certificate: %v", err)
	}
	rootPrivKeyFile, err := os.ReadFile(path.Join(*outputPath, "test_root_ca_private_key.pem"))
	rootPrivKeyData := []byte(strings.ReplaceAll(string(rootPrivKeyFile), "TESTING KEY", "PRIVATE KEY"))
	block, rootCertFile = pem.Decode(rootPrivKeyData)
	...
	if err != nil {
		klog.Fatalf("Failed to read root CA private key: %v", err)
	}
